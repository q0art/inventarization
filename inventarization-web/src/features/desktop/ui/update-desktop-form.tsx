...
import { UpdateDesktopSchema } from "@/features/desktop";
...
import { Desktop, UpdateDesktopDto } from "@/entities/desktop";
import {
  useGetAllCasesQuery,
...
interface Props extends Omit<Desktop, "createdAt" | "updatedAt"> {
  isError: boolean;
  error: unknown;
  onUpdate: (data: { id: string; dto: UpdateDesktopDto }) => void;
}

export const UpdateDesktopForm: FC<Props> = ({
  id,
  name,
  cpuId,
  gpuId,
  ...
  const form = useForm({
    resolver: zodResolver(UpdateDesktopSchema),
    defaultValues: {
      name: name || "",
      cpuId: cpuId || "",
      gpuId: gpuId || "",
  ...
  const onSubmit = async (values: z.infer<typeof UpdateDesktopSchema>) => {
    onUpdate({ id, dto: values });
...
UpdateDesktopForm.displayName = "update-desktop-form";
